...
export function PlatformManager() {
  ...
  async function createSalon(event: FormEvent<HTMLFormElement>) {
    ...
    try { await apiRequest("platform/salons", { method: "POST", body: JSON.stringify({
      ...
      city: form.get("city"), countryCode: String(form.get("countryCode")).toUpperCase(), timezone: form.get("timezone"),
  ...
  return <div className="mt-10 space-y-8">
    ...
    <form onSubmit={createSalon} className="rounded-2xl border border-stone-200 p-6"><h2 className="text-xl font-bold">Add hair salon</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
      <Field name="name" label="Salon name" /><Field name="slug" label="URL name" placeholder="central-salon" /><Field name="streetAddress" label="Address" /><Field name="city" label="City" />
      <Field name="countryCode" label="Country code" defaultValue="IR" maxLength={2} /><Field name="timezone" label="Time zone" defaultValue="Asia/Tehran" />
